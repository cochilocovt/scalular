...
const glassButtonVariants = cva(
  "group relative isolate inline-flex items-center justify-center gap-2 rounded-lg cursor-pointer transition-transform duration-300 ease-out tracking-tight disabled:opacity-50 disabled:cursor-not-allowed focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-foreground/50",
  {
    variants: {
      size: {
...
interface GetStartedButtonProps extends React.ButtonHTMLAttributes<HTMLButtonElement>, VariantProps<typeof glassButtonVariants> {
  label?: string;
  onClick?: () => void;
  className?: string;
...
export const GetStartedButton = React.forwardRef<HTMLButtonElement, GetStartedButtonProps>(
  ({ label = 'Get Started', onClick, className, size = 'lg', href, target, withLamp = false, baseColor = '#043377', icon, ...props }, ref) => {
    ...
    const lampGlow = withLamp && (
      <div className="absolute -top-[70px] left-1/2 flex w-[28rem] lg:w-[50rem] -translate-x-1/2 justify-center pointer-events-none z-0">
        <motion.div 
      ...
      <>
        ...
        <style>{`
          .btn-liquid-${filterId} {
            ...
            /* White text for high contrast on the dark/glass background */
            color: #ffffff;
          ...
          .btn-liquid-lens-${filterId} {
            ...
            box-shadow: 
              ...
              inset -2px -2px 0px -2px color-mix(in srgb, white calc(var(--glass-reflex-light) * 60%), transparent), 
              inset -3px -8px 1px -6px color-mix(in srgb, white calc(var(--glass-reflex-light) * 40%), transparent), 
              inset -0.3px -1px 4px 0px color-mix(in srgb, black calc(var(--glass-reflex-dark) * 30%), transparent), 
              inset -1.5px 2.5px 0px -2px color-mix(in srgb, black calc(var(--glass-reflex-dark) * 40%), transparent), 
          ...
          @media (hover: hover) {
            ...
            .btn-liquid-${filterId}:not(:disabled):hover .btn-liquid-lens-${filterId} {
               ...
               box-shadow: 
                 ...
                 inset -2px -2px 0px -2px color-mix(in srgb, white calc(var(--glass-reflex-light) * 70%), transparent), 
                 inset -3px -8px 1px -6px color-mix(in srgb, white calc(var(--glass-reflex-light) * 50%), transparent), 
                 inset -0.3px -1px 4px 0px color-mix(in srgb, black calc(var(--glass-reflex-dark) * 40%), transparent), 
                 inset -1.5px 2.5px 0px -2px color-mix(in srgb, black calc(var(--glass-reflex-dark) * 50%), transparent), 
        ...
        <span className={`btn-liquid-text-${filterId} relative z-10 w-full flex items-center justify-center select-none`}>
          {icon && <span className="relative z-20 -ms-1 me-2 flex items-center justify-center">{icon}</span>}
          <span className="relative z-20 font-semibold tracking-tight">
            {label}
          </span>
          <ArrowRight
            className={cn(
              "relative z-20 -me-1 ms-2 opacity-60 transition-transform group-hover:translate-x-0.5",
              size === 'sm' ? "w-3.5 h-3.5 md:w-4 md:h-4" : "w-4 h-4 md:w-[18px] md:h-[18px]"
            )}
            strokeWidth={3}
            aria-hidden="true"
          />
    ...
    const innerComponent = (
      <div className={cn("relative inline-block outline-none isolate", className)}
        ...
        {href ? (
          <Link 
            ...
            className={cn(glassButtonVariants({ size }), `btn-liquid-${filterId}`, "outline-none")}
          >
            {buttonContent}
          </Link>
        ) : (
          <button 
            ref={ref}
            onClick={onClick} 
            className={cn(glassButtonVariants({ size }), `btn-liquid-${filterId}`, "outline-none w-full")} 
            aria-label={label}
            {...props}
          >
            {buttonContent}
